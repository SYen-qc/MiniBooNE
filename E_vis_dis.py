...
POT = 18.75 * 1e20
#POT = 11.27 * 1e20
me = 0.5109989461 * 1e-3
mmu = 105.6583745 * 1e-3
mK = 493.677 * 1e-3
mN = 35 * 1e-3
x_l = mmu/mK
x_N = mN/mK
...
p_a_sum = 0
for i in range(count_p_a):
    p_a_sum = p_a_sum + p_a[i,1]

#fp_a = InterpolatedUnivariateSpline(p_a[:,0], p_a[:,1], k=1)
fp_a = interp1d(p_a[:,0], p_a[:,1], kind='linear')
#fp_a = InterpolatedUnivariateSpline(p_a_norm[:,0], p_a_norm[:,1], k=1)
#pa_total = integrate.quad(lambda x:fp_a(x), 0, 5.5, limit=60, full_output=1)[0]
xp_a = np.linspace(0, 5.5, num=5500, endpoint=True)

for i in range(5501):
   p_anew[i,0] = i * 1/1000
   p_anew[i,1] = fp_a(i * 1/1000)
   #p_anew[i,1] = fp_a(i * 1/1000)/pa_total
fp_a_fin = InterpolatedUnivariateSpline(p_anew[:,0], p_anew[:,1], k=1)
...
def z(x,y):
    return (2*np.sqrt(ma**2+y**2)*EN(x)-ma**2-mN**2)/(2*y*x)
#print(z(1,1))

def dz_dpa(x,y):
    return abs((np.sqrt(ma**2+y**2)*(ma**2+mN**2)-2*EN(x)*ma**2)/(2*np.sqrt(ma**2+y**2)*y**2*x))

def integrand_neu(x,y):
    if ((mN**2 - ma**2)**2 - 4*ma**2*x**2*(1 - z(x,y)**2)) >= 0:
        if R(x) >= 1:
            #if 1.25 >= y >= pa_min:
            if y >= pa_min:
                if 1 >= z(x,y) >= -1:
                    return phi_neu_fun(x)*P_N_dec(x)*w_time(x)*P_a_dec(y) * (1/ΓN_lab(x)) * ((cN**2 * U_l4_sq)/(128*np.pi*fa**2)) * ((mN**2 * (mN**2 - ma**2) * y**2)/(EN(x) * np.abs(y * EN(x) - x * np.sqrt(ma**2+y**2) * z(x,y)))) * dz_dpa(x,y) * eff_fun(y) * p_a_fun(y)
                else:
                    return 0
            elif y < pa_min:
                return 0
            ...
            #if 1.25 >= y >= pa_min:
            if y >= pa_min:
                if 1 >= z(x,y) >= np.sqrt(1 - R(x)**2) or -np.sqrt(1 - R(x)**2) >= z(x,y) >= -1:
                    return phi_neu_fun(x)*P_N_dec(x)*w_time(x)*P_a_dec(y) * (1/ΓN_lab(x)) * ((cN**2 * U_l4_sq)/(128*np.pi*fa**2)) * ((mN**2 * (mN**2 - ma**2) * y**2)/(EN(x) * np.abs(y * EN(x) - x * np.sqrt(ma**2+y**2) * z(x,y)))) * dz_dpa(x,y) * eff_fun(y) * p_a_fun(y)
                else:
                    return 0
            elif y < pa_min:
            ...
            return 0
    elif ((mN**2 - ma**2)**2 - 4*ma**2*x**2*(1 - z(x,y)**2)) < 0:
        return 0
    else:
        return 0
...
xnew = np.arange(100, 1250, 50)
partial_int_neu = lambda y: integrate.quad(integrand_neu, PN0, 5, args=(y,), limit=60, full_output=1)[0]
partial_N = lambda y: integrate.quad(N, PN0, 5, args=(y,), limit=60, full_output=1)[0]


def result(E):
    if  partial_N(np.sqrt(E**2-ma**2)) > 0:
        return POT*rho*AMB*partial_int_neu(np.sqrt(E**2-ma**2))/partial_N(np.sqrt(E**2-ma**2))
    else:
        return 0

integration_bin = np.empty([23,2])
for i in range(23):
    integration_bin[i,0] = 125 + i*50
    integration_bin[i,1] = integrate.quad(lambda E:result(E), 0.1 + i*0.05, 0.1 + (i+1)*0.05, limit=60, full_output=1)[0]
...
E_vis_data = np.loadtxt('/Users/shihyentseng/Documents/ＭiniBooNE/E_vis_data.txt')
E_vis_data_array = np.empty([23,2])
for i in range(23):
    E_vis_data_array[i,0] = 125 + i*50
    E_vis_data_array[i,1] = E_vis_data[i,1]

E_vis_sim = np.loadtxt('/Users/shihyentseng/Documents/ＭiniBooNE/E_vis_sim.txt')
E_vis_sim_array = np.empty([25,2])
E_vis_sim_array[0,0] = 100
E_vis_sim_array[0,1] = E_vis_sim[0,1]
E_vis_sim_array[24,0] = 1250
E_vis_sim_array[24,1] = E_vis_sim[22,1]
for i in range(23):
    E_vis_sim_array[i+1,0] = 125 + i*50
    E_vis_sim_array[i+1,1] = E_vis_sim[i,1]

E_vis_excess = np.empty([25,2])
E_vis_excess[0,0] = 100
E_vis_excess[0,1] = integration_bin[0,1] + E_vis_sim[0,1]
E_vis_excess[24,0] = 1250
E_vis_excess[24,1] = integration_bin[22,1] + E_vis_sim[22,1]
for i in range(23):
    E_vis_excess[i+1,0] = 125 + i*50
    E_vis_excess[i+1,1] = integration_bin[i,1] + E_vis_sim[i,1]

plt.figure(figsize=(10.0, 9.0))
#main results
plt.scatter(E_vis_data_array[:,0], E_vis_data_array[:,1],c='k')
plt.fill_between(E_vis_sim_array[:,0], E_vis_sim_array[:,1] + 1, step="mid", alpha=0.5)
plt.step(E_vis_excess[:,0], E_vis_excess[:,1] + 1, where='mid', c='k', linestyle='dashed')
plt.xticks(fontsize=20, rotation=0)
plt.yticks(fontsize=20, rotation=0)
plt.xlabel('$E_a$ (MeV)', fontsize=25)
...
#plt.savefig('/Users/shihyentseng/Documents/ＭiniBooNE/E_vis_dis_v2.pdf', dpi=300)
plt.show()
